...
void Segmentation_mixt::Init(double *Data, double *param)
{
  ...
  memcpy(_phi,param,sizeof(double)*3*_Pclusters);
  ...
  for (int i = 0; i < _lmax - _lmin+1; i++)
    for (int j = 0; j < i + 1; j++)
    {
      ...
      double *dkp=new double[_Pclusters];
      double *fyk=new double[_Pclusters];
      for (int p=0; p<_Pclusters; p++) 
        dkp[p] = (ykbar - _phi[p])*(ykbar - _phi[p]);


      for (int p=0; p<_Pclusters; p++) 
      {
        fyk[p] = 0.5* ni * (-(dkp[p]+wk)/(_phi[_Pclusters+p]*_phi[_Pclusters+p]) -log(2*pi*_phi[_Pclusters+p]*_phi[_Pclusters+p]) ) + log(_phi[2*_Pclusters+p]);
      }
      double max = fyk[0];
      for (int p=1; p<_Pclusters; p++)
      {
      if (fyk[p]>max) 
        max = fyk[p];
      }
...
void
compute_segmentation_mixt(gsl_vector *x, int Kmax, int lmin, int lmax, gsl_vector *phi, int P, gsl_vector **J_est, gsl_matrix **t_est) {
	...
	if (J_est != NULL) {
		(*J_est)=gsl_vector_alloc(Kmax);
		for (int k=0; k<Kmax; k++)
		  {
		    int t = Min((x->size), (k+1)*lmax);
		    gsl_vector_set((*J_est),k,Seg_mixt._D[k][t-(k+1)*lmin]);
		  }
	}

	(*t_est)=gsl_matrix_calloc(Kmax,Kmax);
	for (int c=0;c<Kmax;c++)
		for (int r=0;r<Kmax;r++)
			if (r>=c) {
				gsl_matrix_set((*t_est),r,c,Seg_mixt._BestBreaks[r][c]+1);
			}
