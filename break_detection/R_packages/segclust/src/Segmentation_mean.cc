...
void
compute_segmentation_mean(gsl_vector *x, int Kmax, int lmin, int lmax, bool vh, gsl_vector **J_est, gsl_matrix **t_est) {
	...
	if (J_est != NULL) {
		...
		for (int k=0; k<Kmax; k++){	        
			...
			gsl_vector_set((*J_est),k,Seg_mean._D[k][t-(k+1)*lmin]);
	   	}
	}
	...
	for (int c=0;c<Kmax;c++)
		for (int r=0;r<Kmax;r++)
			if (r>=c) {
				gsl_matrix_set((*t_est),r,c,Seg_mean._BestBreaks[r][c]+1);
			}
